...
import com.easy.wallet.send.navigation.SEND_OVERVIEW_ROUTE
import com.easy.wallet.send.navigation.SEND_PENDING_ROUTE
...
internal class SendSharedViewModel(
  ...
  private val amount = TextFieldState("0")

  // prep information for sending coin
  val basicInfoUiState = coinBalanceUseCase(coinId)
    .map { assetBalance ->
  ...
  private fun signAndPush(chainId: String, onCompletion: (Throwable?) -> Unit) {
    with(_overviewUiState.value) {
      transactionSigningUseCase(
        ...
        amount = this@SendSharedViewModel.amount.text.toString(),
        fee = selectedFee!!
      ).onEach {
        println("===== $it")
      }.onCompletion { cause ->
    ...
    when (event) {
      ...
      is SendUiEvent.EnterDigital -> {
        amount.edit {
          when {
            ...
            event.char != "." && amount.text.contentEquals("0") -> replace(
              0,
              1,
              event.char
            )
      ...
      SendUiEvent.BuildTransactionPlan -> {
        buildPlan {
          if (it != null) {
            ...
            dispatchEvent(SendUiEvent.NavigateTo(SEND_OVERVIEW_ROUTE))
          }
        }
      }
      ...
      is SendUiEvent.OnSigningTransaction -> {
        ...
        signAndPush(event.chainIdHex ?: "0xaa36a7") {
          if (it == null) {
            dispatchEvent(SendUiEvent.NavigateTo(SEND_PENDING_ROUTE))
          } else {
            dispatchEvent(
