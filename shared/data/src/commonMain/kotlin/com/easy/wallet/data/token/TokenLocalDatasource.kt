...
internal class TokenLocalDatasource(
    ...
    suspend fun loadTokens() = withContext(dispatcher) {
        blockChainQueries.transactionWithResult {
            val chainToken = blockChainQueries.selectByName(chainName = "Ethereum").executeAsList()
                .map { it.toExternalToken() }
            val tokens = blockChainQueries.selectWithTokens("Ethereum").executeAsList()
                .map(LocalToken::toExternalToken)
            chainToken + tokens
        }
    }
