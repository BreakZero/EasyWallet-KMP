...
class CoinGeckoApiController internal constructor(
  ...
  override suspend fun getCoinsMarkets(
    ...
    coinIds: String?
  ): List<CoinMarketInformation> = httpClient
    .tryGet<List<CoinGeckoMarketsDto>>("coins/markets", isThrows = true) {
  ...
  override suspend fun getCoinMarketChart(
    ...
    currency: String,
    days: String
  ): Any? = httpClient.tryGet<CoinGeckoMarketChartDto>("coins/$coinId/market_chart") {
    parameter("vs_currency", currency)
    parameter("days", days)
  }

  override suspend fun getSearchTrending(): List<CoinInformation> = httpClient
    .tryGet<CoinGeckoSearchTrendingDto>("search/trending")
    ?.coins
    ?.map {
      it.item
    }?.map {
