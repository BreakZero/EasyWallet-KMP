...
internal class CoinDaoImpl(
  ...
  override suspend fun findAllCoin(): List<BasicCoin> = queries.findAllCoins().executeAsList().map(FindAllCoins::asPublish)

  override fun findAllStream(): Flow<List<BasicCoin>> = queries
    .findAllCoins()
    .asFlow()
    .mapToList(dispatcher)
    .map { it.map(FindAllCoins::asPublish) }

  override fun findAllByPlatformStream(platformId: String): Flow<List<BasicCoin>> = queries
    .findAllCoinsInPlatform(platformId)
    .asFlow()
    .mapToList(dispatcher)
    .map { it.map(FindAllCoinsInPlatform::asPublish) }

  override fun allCoinStream(): Flow<List<BasicCoin>> = queries
    .findAllCoins()
    .asFlow()
    .mapToList(dispatcher)
    .map { it.map(FindAllCoins::asPublish) }
  ...
  override fun findCoinsByIdStream(id: String): Flow<List<BasicCoin>> = queries.findCoinById(id).asFlow().mapToList(dispatcher).map {
    it.map(FindCoinById::asPublish)
  }
}

private fun FindCoinById.asPublish(): BasicCoin = BasicCoin(
  id = id,
  symbol = symbol,
  name = name,
  ...
  platform = AssetPlatform(
    ...
    network = EvmNetworkInformation
      .decodeFromString(evm_network_info)
      ?.asPublish()
  )
)

private fun FindAllCoins.asPublish(): BasicCoin = BasicCoin(
  id = id,
  symbol = symbol,
  decimalPlace = decimal_place,
  name = name,
...
private fun FindAllCoinsInPlatform.asPublish(): BasicCoin = BasicCoin(
  id = id,
  symbol = symbol,
  decimalPlace = decimal_place,
  name = name,
