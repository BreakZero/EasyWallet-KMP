...
private fun initSupportedPlatform(driver: SqlDriver) {
    driver.execute(
        ...
        """
            ...
            VALUES('ethereum', '1', 'Ethereum', 0, '{"networkName":"ethereum","rpcUrl":"https://mainnet.infura.io/","explorerUrl":null}');
        """.trimIndent(),
        parameters = 0
    )
    driver.execute(
        ...
        """
            INSERT OR IGNORE INTO AssetPlatform(id, chain_identifier, short_name, is_testnet, evm_network_info)
            VALUES('ethereum_sepolia', '11155111', 'Ethereum(Sepolia)', 1, '{"networkName":"ethereum(sepolia)","rpcUrl":"https://sepolia.infura.io/","explorerUrl":null}');
        """.trimIndent(),
        parameters = 0
    )
}
...
private fun initDefaultCoins(driver: SqlDriver) {
    ...
    /*driver.execute(
        ...
        """
            INSERT OR IGNORE INTO CoinEntity(id, platform_id, symbol, name, logo_uri, contract, decimal_place, is_active)
            VALUES('ethereum','ethereum_sepolia','ETH','Ethereum','https://assets.coingecko.com/coins/images/279/small/ethereum.png',NULL,18 ,1);
        """.trimIndent(),
        parameters = 0
    )
    driver.execute(
        ...
        """
            INSERT OR IGNORE INTO CoinEntity(id, platform_id, symbol, name, logo_uri, contract, decimal_place, is_active)
            VALUES('dai','ethereum_sepolia','DAI','Dai','https://assets.coingecko.com/coins/images/9956/small/Badge_Dai.png', '0x3e622317f8C93f7328350cF0B56d9eD4C620C5d6',18 ,1);
        """.trimIndent(),
        parameters = 0
    )
    driver.execute(
