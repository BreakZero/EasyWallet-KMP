...
class LocalAssetRepository internal constructor(
  ...
  PlatformRepository {
  override fun findAllCoinStream(): Flow<List<BasicCoin>> = coinDao.findAllStream()

  override suspend fun findAllCoin(): List<BasicCoin> = coinDao.findAllCoin()

  override suspend fun findCoinById(coinId: String): BasicCoin? = coinDao.findCoinById(coinId)

  override suspend fun findCoinsById(coinId: String): List<BasicCoin> = coinDao.findCoinsById(coinId)
  ...
  override fun allPlatformStream(): Flow<List<AssetPlatform>> = platformDao.findAllStream()

  override fun findPlatformByIdStream(platformId: String): Flow<AssetPlatform?> = platformDao.findByIdStream(platformId)
}
