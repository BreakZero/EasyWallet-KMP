...
class GetUniqueCoinUseCase internal constructor(
    private val walletRepository: MultiWalletRepository,
    private val coinRepository: CoinRepository
) {
    @NativeCoroutines
    operator fun invoke(coinId: String, platformId: String): Flow<AssetCoin> {
        return flow {
            val wallet = walletRepository.findWallet()
                ?: throw NoSuchElementException("No wallet had been set yet.")
            val coin = coinRepository.findUniqueCoin(coinId, platformId)
                ?: throw NoSuchElementException("No coin be found, id is: $coinId in platform $platformId")
            val hdWallet = HDWallet(wallet.mnemonic, wallet.passphrase)
            val (address, keyData) = getAddressAndKey(hdWallet, coin.platform)
            emit(AssetCoin.copyFromBasicCoin(coin, address, keyData))
        }
    }
}
